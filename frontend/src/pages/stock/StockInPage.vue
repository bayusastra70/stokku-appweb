...
import { productApi, supplierApi, stockApi, toCamel } from '@/services/api'
import type { Product, Supplier } from '@/types'

const router = useRouter()

// Product & Supplier lists
...
const reasons = [
  { value: 'purchase', label: 'Pembelian dari Supplier' },
  { value: 'return', label: 'Return dari Customer' },
...
onMounted(async () => {
  try {
    ...
    suppliers.value = (Array.isArray(sRes) ? sRes : []).map((s: any) => toCamel<Supplier>(s))
  } catch (err) {
...
async function handleSubmit() {
  try {
    ...
    setTimeout(() => {
      ...
      form.value = { productId: '', quantity: 1, reason: 'purchase', supplierId: '', notes: '' }
    ...
    errorMsg.value = err.message || 'Gagal mencatat stok masuk'
  } finally {
  ...
  <MainLayout>
    <div class="max-w-2xl mx-auto space-y-6">
      ...
      <div class="flex items-center gap-4">
        ...
        <div>
          <h2 class="text-xl font-bold text-slate-800 dark:text-white">Barang Masuk</h2>
          <p class="text-sm text-slate-400 dark:text-white/40 mt-0.5">Catat penerimaan barang ke inventaris</p>
        </div>
      </div>
      ...
      <template v-else>
        ...
        <div v-if="showSuccess" class="flex items-center gap-3 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl px-5 py-4">
          ...
          <span class="text-sm font-medium text-emerald-700 dark:text-emerald-400">Stok masuk berhasil dicatat!</span>
        </div>
        <!-- Error -->
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="rounded-xl border border-slate-200 dark:border-white/[0.06] bg-white dark:bg-white/[0.03] p-6">
            <h3 class="text-sm font-semibold text-slate-800 dark:text-white mb-4 flex items-center gap-2">
              <svg class="text-emerald-500" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12l7 7 7-7"/></svg>
              Data Barang Masuk
            </h3>
            <div class="space-y-5">
              <!-- Product -->
              ...
              <div v-if="selectedProduct" class="bg-slate-50 dark:bg-white/[0.03] rounded-lg p-4 flex items-center gap-4">
                ...
                <div class="flex-1">
                  ...
                  <p class="text-xs text-slate-400 dark:text-white/30">{{ selectedProduct.categoryName }} • {{ formatRupiah(selectedProduct.price) }}</p>
                </div>
                <div class="text-right">
                  <p class="text-[11px] text-slate-400 dark:text-white/30">Stok saat ini</p>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <label class="block text-xs font-medium text-slate-500 dark:text-white/50 mb-1.5">Jumlah Masuk *</label>
          ...
          <div class="flex items-center justify-end gap-3">
            ...
            <button type="submit" :disabled="isSaving"
              class="inline-flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white font-semibold text-sm rounded-xl hover:opacity-90 disabled:opacity-50 transition-all shadow-lg shadow-emerald-500/25 cursor-pointer">
              ...
              {{ isSaving ? 'Menyimpan...' : '✓ Catat Barang Masuk' }}
            </button>
          </div>
        </form>
      </template>
